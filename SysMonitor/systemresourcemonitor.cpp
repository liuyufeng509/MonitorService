...
void SystemResourceMonitor::GetSystemDiskInfo(disk_t& disk)
{
    struct statfs diskInfo;
    char * pach = disk.diskPach;//磁盘挂载路径
    //get disk size info
    statfs(pach,&diskInfo);
    unsigned long long totalBlocks = diskInfo.f_bsize;
    unsigned long long totalSize = totalBlocks * diskInfo.f_blocks;
    disk.totalSize = totalSize>>20;
    unsigned long long freeDisk = diskInfo.f_bfree*totalBlocks;
    disk.freeSize = freeDisk>>20;
    disk.usage = (float)(disk.totalSize-disk.freeSize)/(disk.totalSize)*100;

    system("touch disk.txt");
    char* fileSystem = disk.fileSystem;
    string str = "iostat -k ";
    string str1 = fileSystem;
    string fileName  = str+str1+" > disk.txt";
    char*p = (char*)fileName.data();
    system(p);
    char buf[256];
    ...
    float j1 ,j2 ,j3;
    long j4 ,j5 ;
    sscanf(buf,"%s%f%f%f%ld%ld",buf1,&j1,&j2,&j3,&j4,&j5);
    //cout<<j4<<endl;
    //cout<<j5<<endl;
    disk.temp_readSpeed = j4;
    disk.temp_writeSpeed = j5;
    vect_r.push_back(disk.temp_readSpeed);
    vect_w.push_back(disk.temp_writeSpeed);
    //cout<<vect_r.size()<<endl;
    if(vect_r.size() == 10)
    {
        int i = 0;
        long sum = 0;
        for (i =0; i < vect_r.size(); i++)
        {
            sum += vect_r[i];
        }
        disk.readSpeed = sum/10;//KB/S
        sum = 0;
        i = 0;
        for (i = 0; i < vect_w.size(); i++)
        {
            sum += vect_w[i];
        }
        disk.writeSpeed = sum/10;//KB/S
        vect_r.clear();
        vect_w.clear();
        //cout<<disk.readSpeed<<endl;
        //cout<<disk.writeSpeed<<endl;
    }
    fclose(fp);
    system("rm disk.txt");
